<template>
   ...
   import sandText from './AtBox.vue'
   ...
   const logA = () => {
     ...
     const text2 = escape2Html(text)      let list: any = []
   ...
   const getAtUser = () => {
     ...
     if (match && match.length === 2) {          return match[1]  //@ 后面跟着的字符 光标停留
     }
     return undefined
   }
   // 创建整个@标签
   ...
   const handleKeyUp = async () => {
     ...
     if (showAt()) {
       ...
       node.value = node1
       endIndex.value = endIndex1
       position.value = await getRangeRect() //获取坐标需要时间，需要异步获取        queryString.value = getAtUser() || ''
